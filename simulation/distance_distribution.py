...
for n_cell in range(1, 9):
    ...
    print(f"\tBonds average distance: {rv_bonds.mean():.3}")
    print(f"\tBonds 99.73% quartile: {rv_bonds.ppf(.9973):.3}")

    print()

    print(f"\tContacts average distance: {rv_contacts.mean():.3}")
    print(f"\tContacts 99.73% quartile: {rv_contacts.ppf(.9973):.3}")
