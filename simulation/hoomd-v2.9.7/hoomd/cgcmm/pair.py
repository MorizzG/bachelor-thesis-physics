...
class cgcmm(hoomd.md.force._force):
    ...
    def __init__(self, r_cut, nlist):
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            self.cpp_force = _cgcmm.CGCMMForceCompute(
                hoomd.context.current.system_definition, self.nlist.cpp_nlist, r_cut
            )
        else:
            self.nlist.cpp_nlist.setStorageMode(_md.NeighborList.storageMode.full)
            self.cpp_force = _cgcmm.CGCMMForceComputeGPU(
                hoomd.context.current.system_definition, self.nlist.cpp_nlist, r_cut
            )
        ...
        for i in range(0, ntypes):
            for j in range(i, ntypes):
                ...
                if (exponents == 124) or (exponents == "lj12_4") or (exponents == "LJ12-4"):
                    ...
                    raise RuntimeError(
                        "Unknown exponent type.  Must be one of MN, ljM_N, LJM-N with M+N in 12+4, 9+6, or 12+6"
                    )
