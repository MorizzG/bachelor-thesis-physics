...
from hoomd.dem import _dem, params, utils
...
class _DEMBase:
    ...
    def setParams2D(self, type, vertices, center=False):
        ...
        rcutmax = 2 * (sqrt(max(x * x + y * y for (x, y) in vertices)) + self.radius * 2 ** (1.0 / 6))
    ...
    def setParams3D(self, type, vertices, faces, center=False):
        ...
        # update the neighbor list
        rcutmax = 2 * (sqrt(max(x * x + y * y + z * z for (x, y, z) in vertices)) + self.radius * 2 ** (1.0 / 6))
        self.r_cut = max(self.r_cut, rcutmax)
...
class WCA(hoomd.md.force._force, _DEMBase):
    ...
    def __init__(self, nlist, radius=1.0):
    ...
    def get_rcut(self):
        ...
        r_cut_dict = nl.rcut()
        r_max_dict = {
            typ: sqrt(max(sum(p * p for p in point) for point in self.vertices[typ])) for typ in self.vertices
        }
        for i in range(ntypes):
            for j in range(i, ntypes):
                ...
                r_cut_dict.set_pair(
                    typei,
                    typej,
                    r_max_dict.get(typei, 0) + r_max_dict.get(typej, 0) + self.radius * 2 * 2.0 ** (1.0 / 6),
                )
...
class SWCA(hoomd.md.force._force, _DEMBase):
    ...
    def __init__(self, nlist, radius=1.0, d_max=None):
        hoomd.util.print_status_line()
        friction = None
        ...
        # interparticle cutoff radius, will be updated as shapes are added
        self.r_cut = 2 * 2 * self.radius * 2 ** (1.0 / 6)
        ...
        for i in range(ntypes):
            for j in range(i, ntypes):
                ...
                r_cut_dict.set_pair(
                    typei,
                    typej,
                    r_max_dict.get(typei, 0)
                    + r_max_dict.get(typej, 0)
                    + self.radius * 2 * 2.0 ** (1.0 / 6)
                    + self.d_max
                    - 1,
                )
