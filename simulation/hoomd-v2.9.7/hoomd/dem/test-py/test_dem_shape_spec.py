...
class dem_shape_spec_base(unittest.TestCase):
    ...
    def setup_system(self, cls, shape_params, expected_shapespec, filename, dim):
        ...
        obj = cls(nlist=md.nlist.cell(), radius=0.5)
    ...
    def test_wca_2d(self):
        ...
        expected_shapespec = [
            dict(type="Polygon", rounding_radius=0.5, vertices=sq_verts),
    ...
    def test_wca_2d_with_sphere(self):
        sq_verts = [[-0.5, -0.5], [0.5, -0.5], [0.5, 0.5], [-0.5, 0.5]]
        sph_verts = [[-0.5, -0.5]]
        shape_params = dict(A=dict(vertices=sq_verts), B=dict(vertices=sph_verts))
        expected_shapespec = [
            dict(type="Polygon", rounding_radius=0.5, vertices=sq_verts),
    ...
    def test_wca_3d_with_sphere(self):
        ...
        shape_params = dict(A=dict(vertices=cube_verts, faces=cube_faces), B=dict(vertices=sph_verts, faces=[]))
        expected_shapespec = [
            dict(type="ConvexPolyhedron", rounding_radius=0.5, vertices=cube_verts),
            dict(type="Sphere", diameter=1),
        ]
