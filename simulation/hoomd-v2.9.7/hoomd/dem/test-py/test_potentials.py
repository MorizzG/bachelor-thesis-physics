...
class sphere_sphere(unittest.TestCase):
    def test_potential_wca_2d(self):
        self._test_potential(hoomd.dem.pair.WCA, twoD=True, radius=0.5)

    @not_on_mpi
    def test_potential_swca_2d(self):
        self._test_potential(hoomd.dem.pair.SWCA, twoD=True, radius=0.5)

    def test_potential_wca_3d(self):
        self._test_potential(hoomd.dem.pair.WCA, twoD=False, radius=0.5)

    @not_on_mpi
    def test_potential_swca_3d(self):
        self._test_potential(hoomd.dem.pair.SWCA, twoD=False, radius=0.5)
    ...
    def _test_potential(self, typ, twoD, **params):
        ...
        if hoomd.comm.get_rank() == 0:
            ...
            snap.particles.position[2] = (0, 2 ** (1.0 / 6), 0)
            snap.particles.position[3] = (1, 2 ** (1.0 / 6), 0)
...
class spherocylinder_sphere(unittest.TestCase):
    ...
    def _test_potential(self, typ, twoD, **params):
        ...
        if hoomd.comm.get_rank() == 0:
            snap.particles.position[0] = (0, 0, 0)
            snap.particles.position[1] = (1, 0, 0)
            snap.particles.position[2] = (0, 2 + 2 ** (1.0 / 6), 0)
            snap.particles.position[3] = (1, 2 + 2 ** (1.0 / 6), 0)
...
class shape_sphere(unittest.TestCase):
    ...
    def _test_potential(self, typ, twoD, **params):
        ...
        if hoomd.comm.get_rank() == 0:
            ...
            snap.particles.position[2] = (0, 5 + 2 ** (1.0 / 6), 0)
            snap.particles.position[3] = (3.5, 5 + 2 ** (1.0 / 6), 0)
...
class shape_shape(unittest.TestCase):
    ...
    def _test_potential(self, typ, twoD, **params):
        ...
        if hoomd.comm.get_rank() == 0:
            ...
            snap.particles.position[2] = (0, 5 + 2 ** (1.0 / 6), 0)
            snap.particles.position[3] = (6, 2.5 + 5 + 2 ** (1.0 / 6), dz)
        ...
        if twoD:
            ...
            faces = [[4, 0, 2, 6], [1, 0, 4, 5], [5, 4, 6, 7], [2, 0, 1, 3], [6, 2, 3, 7], [3, 1, 5, 7]]
            potential.setParams("A", vertices, faces, center=False)
