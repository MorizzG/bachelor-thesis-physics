...
def spheroArea(vertices, radius=1.0, factor=1.0):
    ...
    dthetas = [(theta2 - theta1) % (2 * np.pi) for (theta1, theta2) in zip([thetas[-1]] + thetas[:-1], thetas)]
...
def rmax(vertices, radius=0.0, factor=1.0):
...
def _fanTriangles(vertices, faces=None):
    ...
    if faces is None:
        if len(vertices) < 3:
            return
        for tri in ((vertices[0], verti, vertj) for (verti, vertj) in zip(vertices[1:], vertices[2:])):
            yield tri
    else:
        for face in faces:
            for tri in ((vertices[face[0]], vertices[i], vertices[j]) for (i, j) in zip(face[1:], face[2:])):
                yield tri


def massProperties(vertices, faces=None, factor=1.0):
    ...
    if len(vertices[0]) == 2:
        ...
        a_s = [(x1 * y2 - x2 * y1) for ((x1, y1), (x2, y2)) in zip(vertices, shifted)]
        triangleCOMs = [(v0 + v1) / 3 for (v0, v1) in zip(vertices, shifted)]
        COM = np.sum([a * com for (a, com) in zip(a_s, triangleCOMs)], axis=0) / np.sum(a_s)
        vertices -= COM

        shifted = list(vertices[1:]) + [vertices[0]]
        f = lambda x1, x2: x1 * x1 + x1 * x2 + x2 * x2
        Ixyfs = [(f(y1, y2), f(x1, x2)) for ((x1, y1), (x2, y2)) in zip(vertices, shifted)]

        Ix = sum(I * a for ((I, _), a) in zip(Ixyfs, a_s)) / 12.0
        Iy = sum(I * a for ((_, I), a) in zip(Ixyfs, a_s)) / 12.0
    ...
    factors = 1.0 / np.array([6, 24, 24, 24, 60, 60, 60, 120, 120, 120])
