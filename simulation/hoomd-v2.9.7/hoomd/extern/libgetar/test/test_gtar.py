...
class TestGTAR(unittest.TestCase):
    def test_readThenWrite(self, suffix):
        ...
        convertedRecords = {
            "frames/0/otherblah.f32.ind": np.array(records["frames/0/otherblah.f32.ind"], dtype=np.float32)
        }
        ...
        for mode in ["r", "a"]:
            with gtar.GTAR("test" + suffix, mode) as arch:
                for path in records:
                    self.assertTrue(np.all(arch.readPath(path) == convertedRecords.get(path, records[path])))
        ...
        with gtar.GTAR("test" + suffix, "w") as arch:
            for path in records:
                ...
                for readpath in sorted(written):
                    self.assertTrue(
                        np.all(arch.readPath(readpath) == convertedRecords.get(readpath, records[readpath]))
                    )
...
TestGTAR = MultiSuffixMeta(TestGTAR.__name__, TestGTAR.__bases__, dict(TestGTAR.__dict__))
