...
def test_function_signatures(doc):
    ...
    assert doc(m.KWClass.foo0) == "foo0(self: m.kwargs_and_defaults.KWClass, arg0: int, arg1: float) -> None"
    assert doc(m.KWClass.foo1) == "foo1(self: m.kwargs_and_defaults.KWClass, x: int, y: float) -> None"
...
def test_args_refcount():
    ...
    assert m.args_kwargs_function(7, 8, myval, a=1, b=myval) == ((7, 8, myval), {"a": 1, "b": myval})
    assert refcount(myval) == expected
