...
class param_dict(dict):
    ...
    def __getitem__(self, key):
        ...
        if not key in type_names:
            ...
            self.mc.initialize_shape_params()
            # add any extra parameters in that exist at this time.
...
class convex_polygon_params(_hpmc.convex_polygon_param_proxy, _param):
    ...
    def make_param(self, vertices, ignore_statistics=False):
        return self.make_fn(self.ensure_list(vertices), float(0.0), ignore_statistics)
...
class convex_spheropolygon_params(_hpmc.convex_spheropolygon_param_proxy, _param):
    ...
    def make_param(self, vertices, sweep_radius=0.0, ignore_statistics=False):
        return self.make_fn(self.ensure_list(vertices), float(sweep_radius), ignore_statistics)
...
class simple_polygon_params(_hpmc.simple_polygon_param_proxy, _param):
    ...
    def make_param(self, vertices, ignore_statistics=False):
        return self.make_fn(self.ensure_list(vertices), float(0), ignore_statistics)
...
class polyhedron_params(_hpmc.polyhedron_param_proxy, _param):
    ...
    def make_param(
        ...
        return self.make_fn(
            [self.ensure_list(v) for v in vertices],
...
class faceted_sphere_params(_hpmc.faceted_ellipsoid_param_proxy, _param):
    ...
    def __str__(self):
        string = "faceted sphere(vertices = {}, normals = {}, offsets = {}, diameter = {})".format(
            self.vertices, self.normals, self.offsets, self.diameter
        )
        return string
...
class sphinx_params(_hpmc.sphinx3d_param_proxy, _param):
    ...
    def __str__(self):
        # should we put this in the c++ side?
        string = "sphinx(centers = {}, diameters = {}, diameter = {})".format(
            self.centers, self.diameters, self.diameter
        )
        return string

    def make_param(self, diameters, centers, ignore_statistics=False, colors=None):
        self.colors = None if colors is None else self.ensure_list(colors)
        return self.make_fn(self.ensure_list(diameters), self.ensure_list(centers), ignore_statistics)
...
class ellipsoid_params(_hpmc.ell_param_proxy, _param):
    ...
    def make_param(self, a, b, c, ignore_statistics=False):
        return self.make_fn(float(a), float(b), float(c), ignore_statistics)
...
class sphere_union_params(_hpmc.sphere_union_param_proxy, _param):
    def __init__(self, mc, index):
        ...
        # we will add this base class later because of the size template
        _param.__init__(self, mc, index)
        self.__dict__.update(dict(colors=None))
    ...
    def __str__(self):
        ...
        string = "sphere union(centers = {}, orientations = {}, diameter = {}, overlap = {}, capacity = {})\n".format(
            self.centers, self.orientations, self.diameter, self.overlap, self.capacity
        )
        ct = 0
...
class convex_spheropolyhedron_union_params(_hpmc.convex_polyhedron_union_param_proxy, _param):
    def __init__(self, mc, index):
        _hpmc.convex_polyhedron_union_param_proxy.__init__(self, mc.cpp_integrator, index)
        # we will add this base class later because of the size templated
        _param.__init__(self, mc, index)
        self.__dict__.update(dict(colors=None))
    ...
    def __str__(self):
        ...
        string = "convex polyhedron union(centers = {}, orientations = {}, overlap = {})\n".format(
            self.centers, self.orientations, self.overlap
        )
        ct = 0
        members = self.members
        for m in members:
...
class faceted_ellipsoid_union_params(_hpmc.faceted_ellipsoid_union_param_proxy, _param):
    ...
    def make_param(
        ...
        members = []
        for i, (cur_vertices, cur_normals, cur_offsets, cur_axes, cur_origin) in enumerate(
            zip(vertices, normals, offsets, axes, origins)
        ):
            member_fn = _hpmc.make_faceted_ellipsoid
            members.append(
                member_fn(
                    self.ensure_list(cur_normals),
