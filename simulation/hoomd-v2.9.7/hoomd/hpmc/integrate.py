...
from hoomd.hpmc import _hpmc, data
...
class mode_hpmc(_integrator):
    ...
    def __init__(self, implicit, depletant_mode=None):
        ...
        self.shape_param = data.param_dict(self)
        # must call initialize_shape_params() after the cpp_integrator is created.
    ...
    def update_forces(self):
        ...
        for (i, type_i) in enumerate(type_names):
            if (
                hasattr(self.shape_param[type_i], "ignore_overlaps")
                and self.shape_param[type_i].ignore_overlaps is not None
            ):
                ...
                for (j, type_j) in enumerate(type_names):
                    if (
                        hasattr(self.shape_param[type_j], "ignore_overlaps")
                        and self.shape_param[type_j].ignore_overlaps is not None
                    ):
                        enable = not (
                            self.shape_param[type_i].ignore_overlaps and self.shape_param[type_j].ignore_overlaps
                        )
    ...
    def initialize_shape_params(self):
        shape_param_type = data.__dict__[self.__class__.__name__ + "_params"]
        # using the naming convention for convenience.
        ...
        if self.implicit:
            ...
            if ntrial is not None:
                if depletant_mode_circumsphere(self.depletant_mode):
                    ...
                    hoomd.context.msg.warning(
                        "ntrial is only supported with depletant_mode='circumsphere'. Ignoring.\n"
                    )
    ...
    def get_counters(self):
        ...
        * *move_count* - Count of the number of trial moves during the run
        ...
        counters = self.cpp_integrator.getCounters(1)
        return dict(
            translate_accept_count=counters.translate_accept_count,
    ...
    def get_d(self, type=None):
        ...
        if type is None:
            ...
            return self.cpp_integrator.getD(
                hoomd.context.current.system_definition.getParticleData().getTypeByName(type)
            )
    ...
    def get_a(self, type=None):
        ...
        if type is None:
            ...
            return self.cpp_integrator.getA(
                hoomd.context.current.system_definition.getParticleData().getTypeByName(type)
            )
    ...
    def get_ntrial(self):
        ...
        if not self.implicit:
            hoomd.context.msg.warning(
                "ntrial only available in simulations with non-interacting depletants. Returning 0.\n"
            )
            return 0
    ...
    def get_nR(self):
        ...
        if not self.implicit:
            hoomd.context.msg.warning(
                "nR only available in simulations with non-interacting depletants. Returning 0.\n"
            )
            return 0
    ...
    def get_configurational_bias_ratio(self):
        ...
        if not self.implicit:
            hoomd.context.msg.warning(
                "Quantity only available in simulations with non-interacting depletants. Returning 0.\n"
            )
            return 0
...
def depletant_mode_circumsphere(depletant_mode):
    if depletant_mode == "circumsphere":
        if hoomd.context.current.on_gpu():
            hoomd.context.msg.warning(
                "depletant_mode='circumsphere' is deprecated on the GPU if using ntrial>0. Compile with ENABLE_HPMC_REINSERT enabled to use this feature.\n"
            )
        return True
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                ...
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSphere(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSphere(
                        hoomd.context.current.system_definition, seed, 1
                    )
            else:
            ...
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUSphere(
                    hoomd.context.current.system_definition, cl_c, seed
                )
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUSphere(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUSphere(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
    ...
    * *ignore_overlaps* (**default: False**) - set to True to disable overlap checks between this and other types with *ignore_overlaps=True*
    ...
    def __init__(self, seed, d=0.1, a=0.1, move_ratio=0.5, nselect=4, restore_state=False):
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            ...
            cl_c = _hoomd.CellListGPU(hoomd.context.current.system_definition)
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUConvexPolygon(
                hoomd.context.current.system_definition, cl_c, seed
            )
    ...
    * *ignore_overlaps* (**default: False**) - set to True to disable overlap checks between this and other types with *ignore_overlaps=True*
    ...
    def __init__(self, seed, d=0.1, a=0.1, move_ratio=0.5, nselect=4, restore_state=False):
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            ...
            cl_c = _hoomd.CellListGPU(hoomd.context.current.system_definition)
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUSpheropolygon(
                hoomd.context.current.system_definition, cl_c, seed
            )
    ...
    * *ignore_overlaps* (**default: False**) - set to True to disable overlap checks between this and other types with *ignore_overlaps=True*
    ...
    def __init__(self, seed, d=0.1, a=0.1, move_ratio=0.5, nselect=4, restore_state=False):
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            ...
            cl_c = _hoomd.CellListGPU(hoomd.context.current.system_definition)
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUSimplePolygon(
                hoomd.context.current.system_definition, cl_c, seed
            )
    ...
    def __init__(
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitPolyhedron(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitPolyhedron(
            ...
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUPolyhedron(
                    hoomd.context.current.system_definition, cl_c, seed
                )
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUPolyhedron(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUPolyhedron(
    ...
    def __init__(
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitConvexPolyhedron(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitConvexPolyhedron(
                ...
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoConvexPolyhedron(
                    hoomd.context.current.system_definition, seed
                )
        else:
            cl_c = _hoomd.CellListGPU(hoomd.context.current.system_definition)
            ...
            if implicit:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUConvexPolyhedron(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUConvexPolyhedron(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
            else:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUConvexPolyhedron(
                    hoomd.context.current.system_definition, cl_c, seed
                )
    ...
    def __init__(
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitFacetedEllipsoid(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitFacetedEllipsoid(
                ...
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoFacetedEllipsoid(
                    hoomd.context.current.system_definition, seed
                )
        else:
            cl_c = _hoomd.CellListGPU(hoomd.context.current.system_definition)
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUFacetedEllipsoid(
                    hoomd.context.current.system_definition, cl_c, seed
                )
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUFacetedEllipsoid(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUFacetedEllipsoid(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
...
class faceted_sphere(faceted_ellipsoid):
    ...
    * *ignore_overlaps* (**default: False**) - set to True to disable overlap checks between this and other types with *ignore_overlaps=True*
    ...
    def __init__(
        ...
        # initialize base class
        super(faceted_sphere, self).__init__(
            seed=seed,
            d=d,
            a=a,
    ...
    def __init__(
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSphinx(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSphinx(
            ...
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUSphinx(
                    hoomd.context.current.system_definition, cl_c, seed
                )
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUSphinx(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUSphinx(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
    ...
    def __init__(
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSpheropolyhedron(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSpheropolyhedron(
                ...
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoSpheropolyhedron(
                    hoomd.context.current.system_definition, seed
                )
        else:
            cl_c = _hoomd.CellListGPU(hoomd.context.current.system_definition)
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUSpheropolyhedron(
                    hoomd.context.current.system_definition, cl_c, seed
                )
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUSpheropolyhedron(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUSpheropolyhedron(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
    ...
    def __init__(
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitEllipsoid(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitEllipsoid(
            ...
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUEllipsoid(
                    hoomd.context.current.system_definition, cl_c, seed
                )
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUEllipsoid(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUEllipsoid(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
    ...
    def __init__(
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSphereUnion(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSphereUnion(
            ...
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUSphereUnion(
                    hoomd.context.current.system_definition, cl_c, seed
                )
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUSphereUnion(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUSphereUnion(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
    ...
    def __init__(
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitConvexPolyhedronUnion(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitConvexPolyhedronUnion(
                ...
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoConvexPolyhedronUnion(
                    hoomd.context.current.system_definition, seed
                )
        else:
            cl_c = _hoomd.CellListGPU(hoomd.context.current.system_definition)
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUConvexPolyhedronUnion(
                    hoomd.context.current.system_definition, cl_c, seed
                )
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUConvexPolyhedronUnion(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUConvexPolyhedronUnion(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
    ...
    def __init__(
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if implicit:
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitFacetedEllipsoidUnion(
                        hoomd.context.current.system_definition, seed, 0
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitFacetedEllipsoidUnion(
                ...
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoFacetedEllipsoidUnion(
                    hoomd.context.current.system_definition, seed
                )
        else:
            cl_c = _hoomd.CellListGPU(hoomd.context.current.system_definition)
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUFacetedEllipsoidUnion(
                    hoomd.context.current.system_definition, cl_c, seed
                )
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUFacetedEllipsoidUnion(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUFacetedEllipsoidUnion(
                        hoomd.context.current.system_definition, cl_c, seed
                    )
