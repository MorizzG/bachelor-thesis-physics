...
import hoomd
import numpy as np
from hoomd import analyze, context, data, hpmc, init, jit, lattice
...
class enthalpic_dipole_interaction(unittest.TestCase):
    ...
    def test_head_to_tail_parallel_union(self):
        ...
        self.patch = jit.patch.user_union(
            mc=self.mc,
            r_cut_iso=self.r_cut,
            code_iso=self.dipole_dipole,
            r_cut=0.0,
            code="return 0.0;",
        )
        self.log = analyze.log(filename=None, quantities=["hpmc_patch_energy"], period=0, overwrite=True)
...
class patch_alpha_user(unittest.TestCase):
    ...
    def test_alphas(self):
        ...
        self.patch.alpha_iso[:] = [-1.0, 2.7, 10]
        np.testing.assert_allclose(self.patch.alpha_iso, [-1.0, 2.7, 10])
...
class patch_alpha_user_union(unittest.TestCase):
    def setUp(self):
        ...
        mc.shape_param.set(
            ...
            centers=[(0, 0, -diameter / 2), (0, 0, diameter / 2)],
            overlap=[0] * 2,
        )
        self.patch = jit.patch.user_union(
            mc=mc,
