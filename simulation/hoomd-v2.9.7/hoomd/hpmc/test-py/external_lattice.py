...
import hoomd
import numpy as np
from hoomd import context, data, hpmc, init
...
class external_field_lattice(unittest.TestCase):
    ...
    def run_test(self, latticep, latticeq, k, kalt, q, qalt, uein, snapshot_s, eng_check, a=0.0, d=0.0):
        ...
        if uein:
            ...
            if hoomd.comm.get_rank() == 0:
                ...
                self.assertLess(abs(leng - eng) / eng, 0.001)
    ...
    def test_lattice(self):
        ...
        dq = np.array([np.cos(theta / 2.0), 0.0, 0.0, np.sin(theta / 2)])
        ddq = np.array([1.0, 0.0, 0.0, 0.0]) - dq
        ...
        self.mc.shape_param.set("A", normals=norms, offsets=offs, vertices=v, diameter=diam, origin=orig)
        self.run_test(
            latticep=lattice3d,
            latticeq=latticeq,
            k=k,
