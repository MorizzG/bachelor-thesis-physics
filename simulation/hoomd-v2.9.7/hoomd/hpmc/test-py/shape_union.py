...
class helper_functions(unittest.TestCase):
    ...
    def test_dimer_overlap(self):
        ...
        Bq = numpy.array(
            [
                [numpy.cos(numpy.pi / 2.0 / 2.0), numpy.sin(numpy.pi / 2.0 / 2.0), 0.0, 0.0],
                [numpy.cos(numpy.pi / 2.0 / 2.0), numpy.sin(numpy.pi / 2.0 / 2.0), 0.0, 0.0],
            ]
        )
...
class shape_union(unittest.TestCase):
    ...
    def test_dumbbells(self):
        ...
        for i in range(num_iter):
            ...
            orientations = numpy.einsum("ij,i->ij", orientations, 1.0 / numpy.sqrt(o2))  # orientations of dumbbells

            dumbbell = numpy.array(
                [[quatRot(q, spheres[0]) + r, quatRot(q, spheres[1]) + r] for r, q in zip(positions, orientations)]
            )
    ...
    def test_dimers(self):
        ...
        for i in range(num_iter):
            ...
            # normalize to unit quaternions
            o2 = numpy.einsum("ij,ij->i", orientations, orientations)
            orientations = numpy.einsum("ij,i->ij", orientations, 1.0 / numpy.sqrt(o2))

            dimer_pos = numpy.array(
                [[quatRot(q, cubes[0]) + r, quatRot(q, cubes[1]) + r] for r, q in zip(positions, orientations)]
            )
