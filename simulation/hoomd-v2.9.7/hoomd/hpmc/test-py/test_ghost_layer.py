...
class test_ghost_layer(unittest.TestCase):
    def test_implicit(self):
        ...
        system = init.read_snapshot(
            data.make_snapshot(N=2, box=data.boxdim(Lx=100, Ly=50, Lz=50), particle_types=["A", "B"])
        )
        ...
        system.particles[0].position = (-0.2, 0, 0)
    ...
    def test_base(self):
        # setup the MC integration
        system = init.read_snapshot(
            data.make_snapshot(N=2, box=data.boxdim(Lx=100, Ly=50, Lz=50), particle_types=["A"])
        )
