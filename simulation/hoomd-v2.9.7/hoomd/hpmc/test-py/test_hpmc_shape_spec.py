...
class hpmc_gsd_shape_spec(unittest.TestCase):
    ...
    def test_sphere(self):
        ...
        self.setup_system(
            cls=hpmc.integrate.sphere,
            shape_params=shape_params,
            expected_shapespec=expected_shapespec,
    ...
    def test_ellipsoid(self):
        ...
        expected_shapespec = [dict(type="Ellipsoid", **shape_params["A"]), dict(type="Ellipsoid", **shape_params["B"])]
        self.setup_system(
            cls=hpmc.integrate.ellipsoid,
            shape_params=shape_params,
            expected_shapespec=expected_shapespec,
            filename=self.tmp_file,
            dim=3,
    ...
    def test_convex_polyhedron(self):
        ...
        expected_shapespec = [
            dict(type="ConvexPolyhedron", rounding_radius=0, **shape_params["A"]),
            dict(type="ConvexPolyhedron", rounding_radius=0, **shape_params["B"]),
        ]
        self.setup_system(
            cls=hpmc.integrate.convex_polyhedron,
            shape_params=shape_params,
            expected_shapespec=expected_shapespec,
            filename=self.tmp_file,
            dim=3,
    ...
    def test_convex_spheropolyhedron(self):
        ...
        self.setup_system(
            cls=hpmc.integrate.convex_spheropolyhedron,
            shape_params=shape_params,
            expected_shapespec=expected_shapespec,
            filename=self.tmp_file,
            dim=3,
    ...
    def test_convex_polygon(self):
        ...
        shape_params = dict(A=dict(vertices=sq_verts), B=dict(vertices=trg_verts))
        expected_shapespec = [
            dict(type="Polygon", rounding_radius=0, vertices=sq_verts),
            dict(type="Polygon", rounding_radius=0, vertices=trg_verts),
        ]
        self.setup_system(
            cls=hpmc.integrate.convex_polygon,
            shape_params=shape_params,
            expected_shapespec=expected_shapespec,
            filename=self.tmp_file,
            dim=2,
    ...
    def test_convex_spheropolygon(self):
        ...
        self.setup_system(
            cls=hpmc.integrate.convex_spheropolygon,
            shape_params=shape_params,
            expected_shapespec=expected_shapespec,
            filename=self.tmp_file,
            dim=2,
    ...
    def test_simple_polygon(self):
        ...
        shape_params = dict(A=dict(vertices=sq_verts), B=dict(vertices=concave_verts))
        expected_shapespec = [
            dict(type="Polygon", rounding_radius=0, vertices=sq_verts),
            dict(type="Polygon", rounding_radius=0, vertices=concave_verts),
        ]
        self.setup_system(
            cls=hpmc.integrate.simple_polygon,
            shape_params=shape_params,
            expected_shapespec=expected_shapespec,
            filename=self.tmp_file,
            dim=2,
