...
def latticeToHoomd(a1, a2, a3=[0.0, 0.0, 1.0], ndim=3):
        ...
        if len(tokens) > 2:  # definition line
            if re.match("^def", line) is not None:
                ...
                if shape == "poly3d":
                    ...
                    t_params[t]["verts"] = np.array([float(n) for n in tokens[4 : num_verts * 3 + 4]]).reshape(
                        num_verts, 3
                    )
                if shape == "spoly3d":
                    ...
                    t_params[t]["verts"] = np.array([float(n) for n in tokens[5 : num_verts * 3 + 5]]).reshape(
                        num_verts, 3
                    )
                if shape == "sphere":
...
class snapshot:
    ...
    def __init__(self):
        ...
        self.tdef = dict()
        colors = [
            "ff" + "".join([str(c) for c in (256 * np.array(cs.hsv_to_rgb(h, 0.7, 0.7)))])
            for h in np.linspace(0.0, 0.8, self.ntypes)
        ]
...
class tune_npt(tune):
    ...
    def __init__(
        ...
        **kwargs
        ...
        hoomd.util.unquiet_status()
        super(tune_npt, self).__init__(
            obj, tunables, max_val, target, max_scale, gamma, type, tunable_map, *args, **kwargs
        )
