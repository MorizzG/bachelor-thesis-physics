...
class sphereEOS_test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log = analyze.log(
            filename=None,
            quantities=["hpmc_overlap_count", "volume", "phi_p", "hpmc_d", "hpmc_a", "time"],
            overwrite=True,
            period=100,
        )
        self.log.register_callback(
            "phi_p", lambda timestep: len(self.system.particles) * V / self.system.box.get_volume()
        )
        ...
        for i in range(10):
            ...
            if comm.get_rank() == 0:
                print(
                    "d: {:3.2f} accept: {:3.2f} / v: {:3.2f} accept: {:3.2f}".format(
                        d, translate_acceptance, v, volume_acceptance
                    )
                )
