...
class analyze_log_tests(unittest.TestCase):
    ...
    def test_set_params(self):
        ...
        hoomd.run(100)
        ana.set_params(quantities=["test4", "test5"], delimiter=",")
        hoomd.run(100)
...
class analyze_log_query_tests(unittest.TestCase):
    def setUp(self):
        init.create_lattice(lattice.sc(a=1.5), n=[8, 8, 8])
...
class analyze_log_hdf5_tests(unittest.TestCase):
    ...
    def test_phase(self):
        ...
        with hoomd.hdf5.File(self.tmp_file, "a") as h5file:
            ana = hoomd.hdf5.log(
                h5file,
                quantities=["test1", "test2", "test3"],
                matrix_quantities=["mtest1", "mtest2"],
                period=10,
                phase=0,
            )
            ana.register_callback("mtest1", callback, matrix=True)
