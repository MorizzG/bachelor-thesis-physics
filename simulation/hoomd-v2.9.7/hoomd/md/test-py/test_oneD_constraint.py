...
class test_oneD(unittest.TestCase):
    def setUp(self):
        ...
        self.system.box = hoomd.data.boxdim(
            Lx=self.system.box.Lx * 2, Ly=self.system.box.Ly * 2, Lz=self.system.box.Lz * 2
        )
    ...
    def test_y(self):
        ...
        dots = np.dot(pos_diff, np.asarray(constraint_vec)) / (
            np.linalg.norm(pos_diff, axis=1) * np.linalg.norm(np.asarray(constraint_vec))
        )
        tolerance = 1e-3
