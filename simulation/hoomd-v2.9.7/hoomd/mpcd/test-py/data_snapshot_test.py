...
class mpcd_snapshot(unittest.TestCase):
    ...
    def test_make_read(self):
        hoomd.init.read_snapshot(hoomd.data.make_snapshot(N=0, box=hoomd.data.boxdim(L=20.0)))
    ...
    def test_take_restore(self):
        ...
        np.testing.assert_array_equal(
            [pdata.getNameByType(i) for i in range(0, len(targets["types"]))], targets["types"]
        )
        np.testing.assert_array_equal(
            [pdata.getTypeByName(t) for t in targets["types"]], range(0, len(targets["types"]))
        )
    ...
    def test_bad_positions(self):
        ...
        snap = mpcd.data.make_snapshot()
        snap.particles.resize(2)
        snap.particles.position[1] = [11.0, 0.0, 0.0]
        with self.assertRaises(RuntimeError):
            mpcd.init.read_snapshot(snap)
