...
class integrate_slit_tests(unittest.TestCase):
    ...
    def test_create(self):
        mpcd.integrate.slit(group=self.group, H=4.0, V=0.1, boundary="no_slip")

    # test for setting parameters
    def test_set_params(self):
        slit = mpcd.integrate.slit(group=self.group, H=4.0)
        self.assertAlmostEqual(slit.H, 4.0)
        self.assertAlmostEqual(slit.V, 0.0)
        self.assertEqual(slit.boundary, "no_slip")
        self.assertAlmostEqual(slit.cpp_method.geometry.getH(), 4.0)
        self.assertAlmostEqual(slit.cpp_method.geometry.getVelocity(), 0.0)
        ...
        slit.set_params(H=2.0)
        self.assertAlmostEqual(slit.H, 2.0)
        self.assertAlmostEqual(slit.V, 0.0)
        self.assertEqual(slit.boundary, "no_slip")
        self.assertAlmostEqual(slit.cpp_method.geometry.getH(), 2.0)
        self.assertAlmostEqual(slit.cpp_method.geometry.getVelocity(), 0.0)
        self.assertEqual(slit.cpp_method.geometry.getBoundaryCondition(), hoomd.mpcd._mpcd.boundary.no_slip)
    ...
    def test_step_noslip(self):
        mpcd.integrate.slit(group=self.group, H=4.0)
    ...
    def test_step_moving_wall(self):
        ...
        mpcd.integrate.slit(group=self.group, H=4.0, boundary="no_slip", V=1.0)
        ...
        if hoomd.comm.get_rank() == 0:
            ...
            np.testing.assert_array_almost_equal(snap.particles.velocity[1], [0.0, 1.0, 1.0])

    # test basic stepping behavior with slip boundary conditions
    def test_step_slip(self):
        mpcd.integrate.slit(group=self.group, H=4.0, boundary="slip")
    ...
    def test_accel(self):
        mpcd.integrate.slit(group=self.group, H=4.0)
        md.force.constant(fx=2.0, fy=-2.0, fz=4.0)
    ...
    def test_validate_box(self):
        ...
        slit = mpcd.integrate.slit(group=self.group, H=10.0)
        with self.assertRaises(RuntimeError):
            hoomd.run(1)

        # now it should be valid
        slit.set_params(H=4.0)
        hoomd.run(2)

        # make sure we can invalidate it again
        slit.set_params(H=10.0)
        with self.assertRaises(RuntimeError):
            hoomd.run(1)
