...
class mpcd_stream_slit_test(unittest.TestCase):
    ...
    def test_create(self):
        mpcd.stream.slit(H=4.0, V=0.1, boundary="no_slip", period=2)

    # test for setting parameters
    def test_set_params(self):
        slit = mpcd.stream.slit(H=4.0)
        self.assertAlmostEqual(slit.H, 4.0)
        self.assertAlmostEqual(slit.V, 0.0)
        self.assertEqual(slit.boundary, "no_slip")
        self.assertAlmostEqual(slit._cpp.geometry.getH(), 4.0)
        self.assertAlmostEqual(slit._cpp.geometry.getVelocity(), 0.0)
        ...
        self.assertAlmostEqual(slit.V, 0.0)
        self.assertEqual(slit.boundary, "no_slip")
        self.assertAlmostEqual(slit._cpp.geometry.getH(), 2.0)
        self.assertAlmostEqual(slit._cpp.geometry.getVelocity(), 0.0)
        self.assertEqual(slit._cpp.geometry.getBoundaryCondition(), mpcd._mpcd.boundary.no_slip)
    ...
    def test_step_moving_wall(self):
        mpcd.stream.slit(H=4.0, boundary="no_slip", V=1.0, period=3)
    ...
    def test_step_slip(self):
        mpcd.stream.slit(H=4.0, boundary="slip")
    ...
    def test_filler(self):
        ...
        slit = mpcd.stream.slit(H=4.0)
        slit.set_filler(density=5.0, kT=1.0, seed=42, type="A")
        ...
        slit.set_filler(density=10.0, kT=1.5, seed=7)
        self.assertTrue(slit._filler is not None)
        hoomd.run(1)

        # assert an error is raised if we set a bad particle type
        with self.assertRaises(RuntimeError):
            slit.set_filler(density=5.0, kT=1.0, seed=42, type="B")
