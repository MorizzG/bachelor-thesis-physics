...
class mpcd_at_validation(unittest.TestCase):
    def setUp(self):
        ...
        self.at = mpcd.collide.at(seed=791, period=1, kT=1.0)
        mpcd.stream.bulk(period=1)
        md.integrate.nve(hoomd.group.all())

    def test_solvent(self):
        """Test momentum conservation for AT solvent."""
