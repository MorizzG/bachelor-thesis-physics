...
try:
    ...
    print("Warning: gtar python module not found, some getar tests not run", file=sys.stderr)
    gtar = None
...
class RandomSystem:
    def __init__(self, seed):
        ...
        self.randomizeTypes()
        self.randomizeScalar("position", self.nParticles, 3, 0.25)
    ...
    def randomizeScalar(self, name, N, width=1, thresh=0.5):
        if random.random() < thresh:
            ...
            setattr(
                self,
                name,
                [(tuple(random.random() for _ in range(width)) if width > 1 else random.random()) for _ in range(N)],
            )
    ...
    def randomizeBonds(self):
        ...
        if self.nBondTypes:
            ...
            self.bonds = [
                (
                    random.randint(0, self.nBondTypes - 1),
                    random.randint(0, self.nParticles - 1),
                    random.randint(0, self.nParticles - 1),
                )
                for _ in range(self.nBonds)
            ]
    ...
    def writeXML(self, filename="test.xml"):
        with open(filename, "w") as inpFile:
            ...
            inpFile.write('<box lx="{x}" ly="{y}" lz={z} />\n'.format(x=self.box[0], y=self.box[1], z=self.box[2]))
            inpFile.write("<position>\n" + "\n".join("{} {} {}".format(*r) for r in self.position) + "</position>\n")
            inpFile.write("<velocity>\n" + "\n".join("{} {} {}".format(*r) for r in self.velocity) + "</velocity>\n")
            inpFile.write(
                "<acceleration>\n" + "\n".join("{} {} {}".format(*r) for r in self.acceleration) + "</acceleration>\n"
            )
    ...
    def __eq__(self, other):
        ...
        for name in ["position", "velocity", "acceleration", "mass", "charge", "diameter", "box"]:
            result = (
                not getattr(self, name)
                or not getattr(other, name)
                or result
                and np.allclose(getattr(self, name), getattr(other, name))
            )
        ...
        def _test_procedure(self, fname):
            ...
            for _ in range(10):
                with hoomd.context.initialize():
                    ...
                    system = hoomd.init.read_getar(fname, {"any": "any"})
...
class test_basic_io(unittest.TestCase):
    def test_basic(self):
        ...
        vel_prop = hoomd.dump.getar.DumpProp("velocity", True, hoomd.dump.getar.Compression.NoCompress)
        for suffix in ["zip", "tar", "sqlite"]:
            fname_suffix = "dump.{}".format(suffix)
            tmp_file = get_tmp(suffix=fname_suffix)

            hoomd.dump.getar.immediate(tmp_file, static=["type"], dynamic=["position", vel_prop])
    ...
    def test_simple(self):
        ...
        for suffix in ["zip", "tar", "sqlite"]:
            ...
            dump = hoomd.dump.getar.simple(
                tmp_file, mode="w", period=1e3, static=["viz_static"], dynamic=["viz_dynamic"]
            )
            hoomd.run(1)
            dump.close()
            dump.disable()
    ...
    def test_periodic(self):
        ...
        for suffix in ["zip", "tar", "sqlite"]:
            ...
            dump = hoomd.dump.getar(tmp_file, mode="w", static=["viz_static"], dynamic={"viz_aniso_dynamic": 1e3})
            hoomd.run(1)
            dump.close()
            dump.disable()
            hoomd.comm.barrier_all()
